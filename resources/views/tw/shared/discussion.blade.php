...
<div class="container mx-auto">
    <div class="my-3 flex">
        <div class="text-center">
            <h1 class="text-2xl uppercase text-secondary">{{$discussion->title}}</h1>
    ...
    <div class="py-3 my-3 border-b border-gray-700">
        <div class="col-auto">
    ...
    <div class="flex justify-between items-base py-3 my-3">
        @if(!$discussion->is_locked)
        <div class="mt-4">
            <div class="inline-block bg-gray-300 p-3 rounded-full" avatar="{{$discussion->user->name}}"></div>
        </div>
        <div class="mt-4 ml-4 w-full">
            <form action="{{route('posts.store')}}" method="POST" id="post-form">
                ...
                <div class="form-group">
                    ...
                    <textarea placeholder="Comenta aqui" class=" w-full active:outline-none focus:outline-none focus:shadow-outline-blue p-4 placeholder-gray-500 rounded-md" name="content" id="post-content" value="{{old('content')}}"></textarea>
                    @if($errors->has('content'))
                    <p class=" text-danger">{{$errors->first('content')}}</p>
                    @endif
                </div>
                <div class="flex justify-end">
                    <button class="mt-4 py-2 px-4 bg-primary-500 rounded-lg text-white shadow-lg" type="submit">Send Answer</button>
                </div>
            </form>
        </div>
        @else
